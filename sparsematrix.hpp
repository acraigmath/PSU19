...
class SparseMatrix : public Operator
{
	...
	/*! @brief Get the diagonal entries (int)
	    @retval diag the diagonal entries
	*/
	//std::vector<int> GetDiagInt() const;
	...
	/*! @brief Return to the Jacobi smoother of a square
		   matrix in sparse form
	    @retval M the Jacobi (diagonal)
	*/
	SparseMatrix<double> GetJacobi();
	
	/*! @brief Create the Symm. Gauss-Seidel smoother
	    @retval M the smoother
	*/
	SparseMatrix<T> GetGaussS();
	...
	/*! @brief Eliminate a row and column and leave the diagonal
		   untouched
	    @param index and row to eliminate
	*/
	void EliminateRowColD(int index);
...
/*
template <typename T>
std::vector<int> SparseMatrix<T>::GetDiagInt() const
{
	assert(rows_ == cols_);
...
template<typename T>
SparseMatrix<double> SparseMatrix<T>::GetJacobi()
{
	
	assert(rows_ == cols_);
	std::vector<double> TempVector = GetDiag();
	SparseMatrix<double> M(TempVector);
	return M;

}

template <typename T>
SparseMatrix<T> SparseMatrix<T>::GetGaussS()
{
	assert(rows_ == cols_);

	//note that these are not strictly upper/lower
	//so they include the diagonal
	SparseMatrix<T> L(*this);
	SparseMatrix<T> U(*this);
	SparseMatrix<T> D(*this);
	...
	for(int i=0; i<cols_; ++i)
	{
		for(int j=indptr_[i]; j<indptr_[i+1]; ++j)
		{
			...
			{
				double val = D.data_[j];
				if(val!=0.0)
				D.data_[j] = D.data_[j]*(1/val)*(1/val);
			}	
		}
	}
...
template <typename T>
void SparseMatrix<T>::EliminateRowColD(int index)
{
    assert(index >= 0);
    assert(index < rows_);
    assert(index < cols_);
